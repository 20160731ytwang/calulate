...
extern ssize_t tcm_show_caps(struct device *, struct device_attribute *attr,
				char *);
extern ssize_t tcm_show_caps_1_2(struct device *, struct device_attribute *attr,
				char *);
...
struct tcm_chip {
	...
	struct semaphore tcm_mutex;	       /* tcm is processing */
...
extern void tcm_gen_interrupt(struct tcm_chip *);
extern void tcm_continue_selftest(struct tcm_chip *);
...
/*
#ifdef CONFIG_ACPI
extern struct dentry ** tcm_bios_log_setup(char *);
extern void tcm_bios_log_teardown(struct dentry **);
#else
static inline struct dentry ** tcm_bios_log_setup(char *name)
{
	return NULL;
}
static inline void tcm_bios_log_teardown(struct dentry **dir)
{
}
#endif*/
