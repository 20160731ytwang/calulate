		     ...
		     BYTE * encOwnerAuth, UINT32 * encSRKAuthLength, BYTE * encSRKAuth)
{
	...
	BYTE digestBlob[TSPI_TCM_TXBLOB_SIZE];
...
// 			return result;
// 		}
		if ((result = tcm_ecc_init())) {
			return result;
		}
		if ((result = tcm_ecc_encrypt(ownerSecret.authdata, sizeof(ownerSecret.authdata),
					      dummyKey.pubKey.key, dummyKey.pubKey.keyLength,
					      encOwnerAuth, encOwnerAuthLength))) {
			free(dummyKey.pubKey.key);
			free(dummyKey.algorithmParms.parms);
			return result;
		}
		LogDebug("encOwnerAuth length: %d", *encOwnerAuthLength);
		LogDebugData(*encOwnerAuthLength, encOwnerAuth);

		if ((result = tcm_ecc_encrypt(srkSecret.authdata, sizeof(srkSecret.authdata), 
					      dummyKey.pubKey.key, dummyKey.pubKey.keyLength,
					      encSRKAuth, encSRKAuthLength))) {
			free(dummyKey.pubKey.key);
			free(dummyKey.algorithmParms.parms);
			return result;
		}
		tcm_ecc_release();
		LogDebug("encSRKAuth length: %d", *encSRKAuthLength);
		LogDebugData(*encSRKAuthLength, encSRKAuth);
...
// 		return result;
	offset = 0;
	Trspi_LoadBlob_UINT32(&offset, TPM_ORD_TakeOwnership, digestBlob);
	Trspi_LoadBlob_UINT16(&offset, TCPA_PID_OWNER, digestBlob);
	Trspi_LoadBlob_UINT32(&offset, *encOwnerAuthLength, digestBlob);
	Trspi_LoadBlob(&offset, *encOwnerAuthLength, digestBlob, encOwnerAuth);
	Trspi_LoadBlob_UINT32(&offset, *encSRKAuthLength, digestBlob);
	Trspi_LoadBlob(&offset, *encSRKAuthLength, digestBlob, encSRKAuth);
	Trspi_LoadBlob(&offset, srkKeyBlobLength, digestBlob, srkKeyBlob);
	if ((result = tcm_sch_hash(offset, digestBlob, digest.digest))) {
		return result;
	}
	

	if ((result = authsess_ap_init(tspContext, TCM_ET_NONE, 0, authSess))) {
		return result;
	}
	
	if ((result = authsess_ap_hmac(*authSess, &digest))) {
		return result;
	}
	LogDebug("ownerAuthdata:");
	LogDebugData(sizeof((*authSess)->HMAC.authdata), (*authSess)->HMAC.authdata);
