...
TSS_RESULT RPC_APCreate(TSS_HCONTEXT tspContext,		/* in */
		    	TCPA_ENTITY_TYPE entityType,		/* in */
			UINT32 entityValue,			/* in */
			TCPA_NONCE nonceOdd,			/* in */
			TCPA_AUTHDATA inHMAC,			/* in */
			TCS_AUTHHANDLE * authHandle,		/* out */
			TCPA_NONCE * nonceEven,			/* out */
			TCPA_SEQ *seq,				/* out */
			TCPA_AUTHDATA * outHMAC)		/* out */
{
	TSS_RESULT result = (TSS_E_INTERNAL_ERROR | TSS_LAYER_TSP);
	struct host_table_entry *entry = get_table_entry(tspContext);
	...
	switch (entry->type) {
		case CONNECTION_TYPE_TCP_PERSISTANT:
			result = RPC_APCREATE_TP(entry, entityType, entityValue, nonceOdd, inHMAC, 
						 authHandle, nonceEven, seq, outHMAC);
			break;
		default:
			break;
	}
