...
loadData(UINT64 *offset, TCSD_PACKET_TYPE data_type, void *data, int data_size, BYTE *blob)
{
	switch (data_type) {
		...
		case TCSD_PACKET_TYPE_NONCE:
			...
			Trspi_LoadBlob(offset, sizeof(TCPA_NONCE), blob, ((TCPA_NONCE *)data)->nonce);
			break;
		case TCSD_PACKET_TYPE_DIGEST:
			//Trspi_LoadBlob(offset, 20, blob, ((TCPA_DIGEST *)data)->digest);
			Trspi_LoadBlob(offset, sizeof(TCPA_DIGEST), blob, ((TCPA_DIGEST *)data)->digest);
			break;
		case TCSD_PACKET_TYPE_AUTH:
			LoadBlob_TCM_AUTH(offset, blob, ((TCM_AUTH_SESSION *)data));
			break;
		...
		case TCSD_PACKET_TYPE_ENCAUTH:
			...
			Trspi_LoadBlob(offset, sizeof(TCPA_ENCAUTH), blob, ((TCPA_ENCAUTH *)data)->authdata);
			break;
		...
		case TCSD_PACKET_TYPE_AUTH_DATA:
			Trspi_LoadBlob(offset, sizeof(TCPA_AUTHDATA), blob, ((TCPA_AUTHDATA *)data)->authdata);
			break;
		default:
	...
	struct tcsd_comm_data *comm)
{
	...
	switch (dataType) {
		...
		case TCSD_PACKET_TYPE_DIGEST:
			...
			break;
		case TCSD_PACKET_TYPE_AUTH:
			UnloadBlob_TCM_AUTH(&offset, comm->buf, ((TCM_AUTH_SESSION *)theData));
			break;
		case TCSD_PACKET_TYPE_UUID:
		...
		case TCSD_PACKET_TYPE_SECRET:
			...
			break;
		case TCSD_PACKET_TYPE_AUTH_DATA:
			Trspi_UnloadBlob(&offset, sizeof(TCPA_AUTHDATA), comm->buf,
					               ((TCPA_AUTHDATA *) (theData))->authdata);
			break;
		default:
