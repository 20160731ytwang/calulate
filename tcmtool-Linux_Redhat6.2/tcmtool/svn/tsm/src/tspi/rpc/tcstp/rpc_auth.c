...
TSS_RESULT
RPC_APCREATE_TP(struct host_table_entry *hte,
		TCPA_ENTITY_TYPE entityType,	/* in */
		UINT32 entityValue,		/* in */
	...
	if (setData(TCSD_PACKET_TYPE_UINT32, 2, &entityValue, 0, &hte->comm))
		return TSPERR(TSS_E_INTERNAL_ERROR);
	if (setData(TCSD_PACKET_TYPE_NONCE, 3, &nonceOdd, 0, &hte->comm))
		return TSPERR(TSS_E_INTERNAL_ERROR);
	if (setData(TCSD_PACKET_TYPE_AUTH_DATA, 4, &inMac, 0, &hte->comm))
		return TSPERR(TSS_E_INTERNAL_ERROR);
	...
	if (result == TSS_SUCCESS) {
		...
		if (getData(TCSD_PACKET_TYPE_NONCE, 1, nonceEven, 0, &hte->comm))
			result = TSPERR(TSS_E_INTERNAL_ERROR);
		if (getData(TCSD_PACKET_TYPE_UINT32, 2, seq, 0, &hte->comm))
			result = TSPERR(TSS_E_INTERNAL_ERROR);
		if (getData(TCSD_PACKET_TYPE_AUTH_DATA, 3, outMac, 0, &hte->comm))
			result = TSPERR(TSS_E_INTERNAL_ERROR);
	}
