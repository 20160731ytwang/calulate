...
{
	Trspi_LoadBlob_UINT32(offset, auth->authHandle, blob);
	Trspi_LoadBlob(offset, sizeof(auth->HMAC), blob, (BYTE *)&auth->HMAC.authdata);
}

void
UnloadBlob_TCM_AUTH(UINT64 *offset, BYTE *blob, TCM_AUTH_SESSION *auth)
{
	Trspi_UnloadBlob_UINT32(offset, &auth->authHandle, blob);
	Trspi_UnloadBlob(offset, sizeof(auth->outHMAC), blob, (BYTE *)auth->outHMAC.authdata);
}
