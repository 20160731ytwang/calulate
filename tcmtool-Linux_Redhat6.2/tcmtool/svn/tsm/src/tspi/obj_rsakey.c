...
// 		}
	...
	switch (initFlags & TSS_KEY_STRUCT_BITMASK) {
		...
		case TSS_KEY_STRUCT_KEY12:
			...
			rsakey->key.hdr.key12.tag = endian16(TPM_TAG_KEY);
	...
	if (initFlags & TSS_KEY_TSP_SRK) {
		rsakey->key.algorithmParms.algorithmID = TCM_ALG_SMS4;
		rsakey->key.algorithmParms.parmSize = TSM_SMK_PARMS_SIZE;
	
		rsakey->key.algorithmParms.parms = calloc(1, rsakey->key.algorithmParms.parmSize);
		...
		offset = 0;
		memset(rsakey->key.algorithmParms.parms, 0, TSM_SMK_PARMS_SIZE);
		Trspi_LoadBlob_UINT32(&offset, TSM_SMK_KEY_SIZE, rsakey->key.algorithmParms.parms);
		Trspi_LoadBlob_UINT32(&offset, TSM_SMK_BLOCK_SIZE, rsakey->key.algorithmParms.parms);	
		Trspi_LoadBlob_UINT32(&offset, TSM_SMK_IV_SIZE, rsakey->key.algorithmParms.parms);
	...
	if ((initFlags & TSS_KEY_TSP_SRK) == TSS_KEY_TSP_SRK) {
		rsakey->key.keyFlags = TPM_VOLATILE;
		rsakey->key.keyUsage = TCM_SMS4KEY_STORAGE;
		rsakey->key.authDataUsage = TPM_AUTH_NEVER;
		rsakey->key.algorithmParms.encScheme = TCM_ES_SMS4_CBC;
		rsakey->key.algorithmParms.sigScheme = TCM_SS_NONE;
	} else if ((initFlags & TSS_KEY_TYPE_MASK) == TSS_KEY_TYPE_SIGNING) {
