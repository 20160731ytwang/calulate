...
int
bios_open(void *source, LOG_HANDLE *handle)
{
	int fd;
	...
	*handle = (LOG_HANDLE)fd;

	return 0;
}

TSS_RESULT
bios_get_entries_by_pcr(LOG_HANDLE handle, UINT32 pcr_index, UINT32 first,
...
TSS_RESULT
bios_get_entry(LOG_HANDLE handle, UINT32 pcr_index, UINT32 *num, TSS_PCR_EVENT **ppEvent)
{
