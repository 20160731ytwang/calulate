...
void
LoadBlob_TCM_Auth_Special(UINT64 *offset, BYTE *blob, TCM_AUTH_SESSION *auth)
{
	LoadBlob_UINT32(offset, auth->authHandle, blob);
	LoadBlob_AUTHDATA(offset, blob, &(auth->HMAC));
}

void
UnloadBlob_TCM_Auth_Special(UINT64 *offset, BYTE *blob, TCM_AUTH_SESSION *auth)
{
	UnloadBlob_UINT32(offset, &auth->authHandle, blob);
	UnloadBlob_AUTHDATA(offset, blob, &auth->HMAC);
}
...
loadData(UINT64 *offset, TCSD_PACKET_TYPE data_type, void *data, int data_size, BYTE *blob)
{
	switch (data_type) {
		...
		case TCSD_PACKET_TYPE_SECRET:
			...
			break;
		case TCSD_PACKET_TYPE_AUTH_DATA:
			LoadBlob(offset, sizeof(TPM_AUTHDATA), blob, ((TPM_AUTHDATA *)data)->authdata);
			break;
		default:
...
// 			UnloadBlob_Auth_Special(&offset, comm->buf, ((TPM_AUTH *) theData));
			if ((old_offset + sizeof(TPM_AUTHHANDLE)
					+ sizeof(TPM_AUTHDATA) > comm->hdr.packet_size))
				return TCSERR(TSS_E_INTERNAL_ERROR);
			UnloadBlob_TCM_Auth_Special(&offset, comm->buf, (TCM_AUTH_SESSION *) theData);
			break;
		case TCSD_PACKET_TYPE_ENCAUTH:
		...
		case TCSD_PACKET_TYPE_SECRET:
			...
			break;
		case TCSD_PACKET_TYPE_AUTH_DATA:
			if (old_offset + sizeof(TPM_AUTHDATA) > comm->hdr.packet_size)
				return TCSERR(TSS_E_INTERNAL_ERROR);

			UnloadBlob(&offset, sizeof(TPM_AUTHDATA), comm->buf,
					((TPM_AUTHDATA *) (theData))->authdata);
			break;
		default:
