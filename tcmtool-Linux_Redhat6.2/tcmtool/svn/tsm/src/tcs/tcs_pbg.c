...
#define TSS_TCM_RQU_BLOB_AUTH_LEN	(sizeof(TPM_AUTHHANDLE) + sizeof(TPM_AUTHDATA))
#define TSS_TCM_RSP_BLOB_AUTH_LEN	(sizeof(TPM_AUTHDATA))
...
tpm_rsp_parse(TPM_COMMAND_CODE ordinal, BYTE *b, UINT32 len, ...)
{
	...
	* return: UINT32 *, BYTE**, 1 optional AUTH*/
	...
	case TPM_ORD_CMK_CreateKey:
	{
		UINT32 *data_len = va_arg(ap, UINT32 *);
		BYTE **data = va_arg(ap, BYTE **);
		TCM_AUTH_SESSION *auth = va_arg(ap, TCM_AUTH_SESSION *);
		va_end(ap);
		...
		if (auth) {
			offset1 = offset2 = len - TSS_TCM_RSP_BLOB_AUTH_LEN;
			UnloadBlob_TCM_Auth(&offset1, b, auth);
		} else
			offset2 = len;
	...
	/* 1 UINT32, 1 32 byte value, 1 UINT32, 1 32 byte value */
	case TCM_ORD_APCreate:
	{
		UINT32 *sessionHandle = va_arg(ap, UINT32 *);
		BYTE *tcmNonce	= va_arg(ap, BYTE *);
		UINT32 *seq = va_arg(ap, UINT32 *);
		BYTE *outMac = va_arg(ap, BYTE *);
		va_end(ap);

		if (!sessionHandle || !tcmNonce || !seq || !outMac) {
			LogError("Internal error for ordinal 0x%x", ordinal);
			return TCSERR(TSS_E_INTERNAL_ERROR);
		}

		offset1 = TSS_TPM_TXBLOB_HDR_LEN;
		UnloadBlob_UINT32(&offset1, sessionHandle, b);
		UnloadBlob(&offset1, TPM_NONCE_SIZE, b, tcmNonce);
		UnloadBlob_UINT32(&offset1, seq, b);
		UnloadBlob(&offset1, TPM_NONCE_SIZE, b, outMac);
		break;
	}
#ifdef TSS_BUILD_CMK
...
tpm_rqu_build(TPM_COMMAND_CODE ordinal, UINT64 *outOffset, BYTE *out_blob, ...)
{
	...
	case TPM_ORD_TakeOwnership:
	{
		...
		TCM_AUTH_SESSION *auth1 = va_arg(ap, TCM_AUTH_SESSION *);
		va_end(ap);
		...
		*outOffset += TSS_TPM_TXBLOB_HDR_LEN;
		...
		LoadBlob_TCM_Auth(outOffset, out_blob, auth1);
		LoadBlob_Header(TPM_TAG_RQU_AUTH1_COMMAND, *outOffset, ordinal, out_blob);
	...
	/* 1 UINT16(entityType), 1 UINT32(entityValue), 1 32 byte value(callerNonce), 1 32 byte value(inMac) */
	case TCM_ORD_APCreate:
	{
		UINT16 entityType = va_arg(ap, int);
		UINT32 entityValue = va_arg(ap, UINT32);
		BYTE *callerNonce = va_arg(ap, BYTE *);
		BYTE *inMac = va_arg(ap, BYTE *);
		va_end(ap);

		if (!callerNonce || !inMac) {
			result = TCSERR(TSS_E_INTERNAL_ERROR);
			LogError("Internal error for ordinal 0x%x", ordinal);
			break;
		}
		...
		*outOffset += TSS_TPM_TXBLOB_HDR_LEN;
		LoadBlob_UINT16(outOffset, entityType, out_blob);
		LoadBlob_UINT32(outOffset, entityValue, out_blob);
		LoadBlob(outOffset, TCM_SCH_256_HASH_LEN, out_blob, callerNonce);
		LoadBlob(outOffset, TCM_SCH_256_HASH_LEN, out_blob, inMac);
		LoadBlob_Header(TPM_TAG_RQU_COMMAND, *outOffset, ordinal, out_blob);

		break;
	}
	/* 1 UINT32(handle), 1 32 byte value(inMac) */
	case TCM_ORD_APTerminate:
	{
		UINT32 handle = va_arg(ap, UINT32);
		BYTE *inMac = va_arg(ap, BYTE *);
		va_end(ap);
		...
		*outOffset += TSS_TPM_TXBLOB_HDR_LEN;
		LoadBlob_UINT32(outOffset, handle, out_blob);
		LoadBlob(outOffset, TCM_SCH_256_HASH_LEN, out_blob, inMac);
		LoadBlob_Header(TPM_TAG_RQU_COMMAND, *outOffset, ordinal, out_blob);
