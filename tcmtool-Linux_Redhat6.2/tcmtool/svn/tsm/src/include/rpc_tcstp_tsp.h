...
TSS_RESULT RPC_APCREATE_TP(struct host_table_entry *hte,TCPA_ENTITY_TYPE entityType,UINT32 entityValue,TCPA_NONCE nonceOdd,TCPA_AUTHDATA inMac,TCS_AUTHHANDLE * authHandle,TCPA_NONCE * nonceEven,TCPA_SEQ *seq,TCPA_AUTHDATA * outMac);
#else
#define RPC_OIAP_TP(...)	TSPERR(TSS_E_INTERNAL_ERROR)
#define RPC_OSAP_TP(...)	TSPERR(TSS_E_INTERNAL_ERROR)
#define RPC_APCREATE_TP(...)	TSPERR(TSS_E_INTERNAL_ERROR)
#endif
...
#ifdef TSS_BUILD_OWN
TSS_RESULT RPC_TakeOwnership_TP(struct host_table_entry *,UINT16,UINT32,BYTE *,UINT32,BYTE *,UINT32,BYTE *,TCM_AUTH_SESSION *,UINT32 *,BYTE **);
