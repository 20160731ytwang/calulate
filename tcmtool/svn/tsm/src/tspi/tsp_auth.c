...
TSS_RESULT
authsess_do_apcreate(TCM_AUTH_SESSION *sess)
{
	TSS_RESULT result;
	LogDebug("authsess_do_apcreate: enter...");
	if ((result = TCS_API(sess->tspContext)->APCreate(sess->tspContext, 
							  sess->entityType, sess->entityValue,
						      	  sess->nonceOdd, sess->inHMAC,
						          &sess->authHandle,
							  &sess->nonceEven,
						          &sess->seq,
						          &sess->outHMAC))) {
		if (result == TCPA_E_RESOURCES) {
			...
			do {
				...
				result = TCS_API(sess->tspContext)->APCreate(sess->tspContext, 
							  		     sess->entityType, sess->entityValue,
						      	                     sess->nonceOdd, sess->inHMAC,
						            		     &sess->authHandle,
							  		     &sess->nonceEven,
	...
	LogDebug("authsess_do_apcreate returns: %d", result);
	LogDebug("auth handle: %08X", sess->authHandle);
	LogDebug("auth seq: %08X", sess->seq);
	LogDebug("nonce even:");
	LogDebugData(sizeof(sess->nonceEven), sess->nonceEven.nonce);
	LogDebug("out hmac:");
	LogDebugData(sizeof(sess->outHMAC.authdata), sess->outHMAC.authdata);

	return result;
}

/* Create an AP session. */
TSS_RESULT
authsess_ap_init(TSS_HCONTEXT     tspContext,
		 TPM_ENTITY_TYPE  entityType,
		 UINT32 entityValue,
		 TCM_AUTH_SESSION  **xsess)
{
 	TSS_RESULT result;
	TCM_AUTH_SESSION *sess;
	UINT64 offset;
	BYTE hmacBlob[3 * sizeof(TPM_DIGEST)];	
	BYTE wellKnownSecret[] = TSS_WELL_KNOWN_SECRET;

	if ((sess = calloc(1, sizeof(TCM_AUTH_SESSION))) == NULL) {
		LogError("malloc of %zd bytes failed", sizeof(TCM_AUTH_SESSION));
		return TSPERR(TSS_E_OUTOFMEMORY);
	}
	
	sess->tspContext = tspContext;
	sess->entityType = entityType;
	sess->entityValue = entityValue;	

	if ((result = get_local_random(tspContext, FALSE, sizeof(TPM_NONCE), (BYTE **)sess->nonceOdd.nonce))) {
		goto error;
	}
	
	if (TCM_ET_NONE == entityType) {
		offset = 0;
		Trspi_LoadBlob(&offset, sizeof(wellKnownSecret), sess->secret.authdata, wellKnownSecret);
	} else {
		if ((result = obj_context_get_policy(tspContext, TSS_POLICY_USAGE, &sess->hPolicy))) {
		return TSPERR(TSS_E_INTERNAL_ERROR);
		}

		if ((result = obj_policy_get_secret(sess->hPolicy, FALSE, &sess->secret))) {
			return TSPERR(TSS_E_INTERNAL_ERROR);
		}
	}
	...
	offset = 0;
	Trspi_LoadBlob_UINT32(&offset, TCM_ORD_APCreate, hmacBlob);
	Trspi_LoadBlob_NONCE(&offset, hmacBlob, &sess->nonceOdd);
	
	tcm_hmac(hmacBlob, offset, sess->secret.authdata, sizeof(sess->secret.authdata), sess->inHMAC.authdata);

	if ((result = authsess_do_apcreate(sess))) {
		goto error;
	}	
...
TSS_RESULT authsess_ap_hmac(TCM_AUTH_SESSION * sess, TPM_DIGEST *digest)
{
 	TSS_RESULT result;
	UINT64 offset;
	BYTE hmacBlob[3 * sizeof(TPM_DIGEST)];	

	if (NULL==sess || NULL==digest) {
		return TSPERR(TSS_E_INTERNAL_ERROR);
	}
	
	offset = 0;
	Trspi_LoadBlob_DIGEST(&offset, hmacBlob, digest);
	Trspi_LoadBlob_UINT32(&offset, sess->seq, hmacBlob);

	result = tcm_hmac(hmacBlob, offset, sess->secret.authdata, sizeof(sess->secret.authdata), sess->HMAC.authdata);
