...
TSS_RESULT
TCSP_APCreate_Internal(TCS_CONTEXT_HANDLE hContext,	/* in */
		      TCPA_ENTITY_TYPE entityType,	/* in */
		      UINT32 entityValue,		/* in */
		      TCPA_NONCE nonceOdd,		/* in */
		      TCPA_AUTHDATA inMac,		/* in */
		      TCS_AUTHHANDLE * authHandle,	/* out */
		      TCPA_NONCE * nonceEven,		/* out */
		      TCPA_SEQ *seq,			/* out */
		      TCPA_AUTHDATA * outMac)		/* out */
{
	UINT64 offset = 0;
	TSS_RESULT result;
	UINT32 paramSize;
	...
	LogDebug("Entering APCreate");
	if ((result = ctx_verify_context(hContext)))
		return result;

	if ((result = tpm_rqu_build(TCM_ORD_APCreate, &offset, txBlob, 
				    entityType, entityValue, nonceOdd.nonce, inMac.authdata)))
		return result;
	...
	result = UnloadBlob_Header(txBlob, &paramSize);
	if (!result) {
		result = tpm_rsp_parse(TCM_ORD_APCreate, txBlob, paramSize, 
				       authHandle, nonceEven->nonce, seq, outMac->authdata);
	}
	LogResult("APCreate", result);

	return result;
}

TSS_RESULT
TCSP_APTerminate_Internal(TCS_CONTEXT_HANDLE hContext,	/* in */
			 ...
			 TCPA_AUTHDATA inMac)		/* in */
{
	...
	LogDebug("Entering APTerminate");
	if ((result = ctx_verify_context(hContext)))
		return result;

	if ((result = tpm_rqu_build(TCM_ORD_APTerminate, &offset, txBlob,
				    handle, inMac.authdata)))
		return result;
