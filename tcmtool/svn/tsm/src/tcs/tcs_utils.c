...
void
LoadBlob_TCM_Auth(UINT64 *offset, BYTE * blob, TCM_AUTH_SESSION * auth)
{
	LoadBlob_UINT32(offset, auth->authHandle, blob);
	LoadBlob(offset, sizeof(auth->HMAC), blob, (BYTE *)&auth->HMAC);
}

void
UnloadBlob_TCM_Auth(UINT64 *offset, BYTE * blob, TCM_AUTH_SESSION * auth)
{
	if (!auth) {
		UnloadBlob(offset, sizeof(auth->outHMAC), blob, NULL);
		return;
	}
	
	UnloadBlob(offset, sizeof(auth->outHMAC), blob, (BYTE*)&auth->outHMAC.authdata);
}
...
{
	LoadBlob(offset, sizeof(TPM_AUTHDATA), blob, authdata->authdata);
}

void
UnloadBlob_AUTHDATA(UINT64 *offset, BYTE *blob, TPM_AUTHDATA *authdata)
{
	UnloadBlob(offset, sizeof(TPM_AUTHDATA), blob, authdata->authdata);
}
