...
{
	// TCM will return an incorrect version[1.0.0.0] for capArea "TPM_CAP_VERSION_VAL"
	// TCPA_CAPABILITY_AREA capArea = TPM_CAP_VERSION_VAL;
	TCPA_CAPABILITY_AREA capArea = TPM_CAP_VERSION;
	UINT32 respSize;
	BYTE *resp;
	TSS_RESULT result;
	UINT64 offset;
...
/* manufacturer in TCM is a string not a UINT32 */
TSS_RESULT
get_cap_manufacturer(BYTE * manufacturer)
{
	UINT32 respSize;
	BYTE *resp;
	TSS_RESULT result;
	...
	result = TCSP_GetCapability_Internal(InternalContext, TPM_CAP_PROPERTY, sizeof(subCap), (BYTE *)&subCap,
					     &respSize, &resp);
	if (!result) {
		offset = 0;
		UnloadBlob(&offset, respSize, manufacturer, resp);
		free(resp);
	}
...
// 		goto err;
	if ((result = get_cap_manufacturer(p->manufacturer)))
		goto err;
