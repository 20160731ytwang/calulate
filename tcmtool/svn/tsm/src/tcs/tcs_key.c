...
TSS_RESULT
UnloadBlob_TSM_KEY(UINT64 *offset, BYTE *blob, TSS_KEY *key)
{
	TSS_RESULT rc;
	...
	if (!key) {
		...
		UnloadBlob_UINT16(offset, NULL, blob);	// tag
		UnloadBlob_UINT16(offset, NULL, blob);	// fill
		UnloadBlob_KEY_FLAGS(offset, blob, NULL);
		UnloadBlob_BOOL(offset, NULL, blob);
		if ((rc = UnloadBlob_KEY_PARMS(offset, blob, NULL)))
			return rc;
...
void
LoadBlob_TSM_KEY(UINT64 *offset, BYTE * blob, TSS_KEY * key)
{
